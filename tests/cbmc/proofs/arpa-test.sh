...
function write_to_log {
    ...
    echo "  1.-PROOF STATS : #deps-in-std=$n_deps_std ($n_stubs_std stubs, $n_deps_unnesc unnesc), #deps-in-arpa=$n_deps_arpa ($n_stubs_arpa stubs)"
    ...
    ##############################
    echo ""
    echo "  3.- START MISSED_DEPENDENCIES (NOT STUBS):"
    echo -n -e "$list_deps_arpa_cannot_find"
    echo "    \-- END MISSED_DEPENDENCIES (NOT STUBS)"
...
for dir in $inp; do
    ...
    code --diff $arpa_log/goto-functions-arpa $arpa_log/goto-functions-std
    cd ..
done 
